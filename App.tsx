...
export default function App() {
      ...
      const aiSearchPromise = (async () => {
        try {
          ...
          const response = await ai.models.generateContent({
            ...
            contents: `Identify up to 3 official POIs for "${qRaw}" located EXCLUSIVELY inside Magdeburg, Germany. Return names only.`,
            config: {
              tools: [{ googleMaps: {} }],
  ...
  // Improved language-sensitive summaries
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#FDFDFD] h-screen overflow-hidden font-sans text-gray-900">
      <div className="w-full md:w-[400px] flex flex-col border-r border-gray-100 bg-white z-20 overflow-hidden shrink-0 shadow-2xl">
            ...
            {searchResults.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200 max-h-[400px] overflow-y-auto custom-scrollbar">
                {searchResults.map((res, idx) => (
                  <button key={res.place_id || idx} onClick={() => selectSearchResult(res)} className="w-full text-left px-5 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-[#93132B] transition-colors border-b last:border-b-0 border-gray-50 flex items-center gap-3 group">
                    <div className="text-xl bg-gray-100 w-10 h-10 rounded-xl flex items-center justify-center shrink-0 group-hover:bg-[#93132B10]">{getCategoryIcon(res)}</div>
        ...
        <DndContext sensors={sensors} onDragStart={(e) => {
              ...
              {pois.length === 0 ? (
                ...
                pois.map(poi => (
                  <POICard 
                    ...
                    onEditToggle={() => { setEditingPoiId(editingPoiId !== poi.id ? poi.id : null); if (editingPoiId !== poi.id) setMapCenter([poi.lat, poi.lng]); }} 
          ...
          <footer className="p-6 bg-white border-t border-gray-100 overflow-y-auto max-h-[45%] custom-scrollbar">
            ...
            <button onClick={() => setIsFinalized(true)} disabled={pois.length === 0} className={`w-full py-4 rounded-2xl font-bold transition-all shadow-xl ${pois.length === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-[#93132B] text-white hover:bg-[#7a0f24]'}`}>
              {t.saveData}
            </button>
          </footer>
      ...
      <div className="flex-1 relative min-h-[400px]">
        <MapContainer center={MAGDEBURG_CENTER} zoom={14} minZoom={12} maxZoom={19} maxBounds={SACHSEN_ANHALT_BOUNDS} maxBoundsViscosity={1.0} style={{ height: '100%', width: '100%' }}>
          <TileLayer key={mapLayer} url={`https://{s}.f4map.com/tiles/${mapLayer}/{z}/{x}/{y}.png`} subdomains={['tile1', 'tile2', 'tile3', 'tile4']} bounds={SACHSEN_ANHALT_BOUNDS} />
          <MapEvents onMapClick={(lat, lng) => setPendingMarkers(prev => prev.some(m => Math.abs(m.lat - lat) < 0.0005) ? prev.filter(m => Math.abs(m.lat - lat) >= 0.0005) : [...prev, { lat, lng }])} />
          ...
          {pois.map(poi => (
            <Marker key={poi.id} position={[poi.lat, poi.lng]} icon={editingPoiId === poi.id ? selectedIcon : (poi.transportMode ? defaultIcon : incompleteIcon)}
              ...
              <Tooltip direction="top" permanent={editingPoiId === poi.id}>
                <div className="px-3 py-2 bg-white rounded-lg shadow-xl border border-gray-100">
                  <p className="font-bold text-[#1a1a1a] truncate max-w-[150px]">{poi.name}</p>
                  <p className="text-xs text-gray-500">{poi.transportMode ? `${TRANSPORT_ICONS[poi.transportMode]} ${t.modes[poi.transportMode]}` : t.modeMissing}</p>
                </div>
              </Tooltip>
            </Marker>
          ))}
        ...
        <div className="absolute top-6 right-20 z-[1000] flex flex-col items-end gap-2">
          {showLayerPicker && (
            <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl p-2 border border-white flex flex-col gap-1">
              {Object.values(MapLayer).map((layer) => (
                <button key={layer} onClick={() => { setMapLayer(layer); setShowLayerPicker(false); }} className={`flex items-center gap-3 px-4 py-2 rounded-xl transition-all ${mapLayer === layer ? 'bg-[#93132B] text-white' : 'hover:bg-gray-100 text-gray-700'}`}>
                  <span className="text-xl">{LAYER_ICONS[layer]}</span>
                  <span className="text-xs font-bold">{layer === MapLayer.STANDARD ? t.layerStandard : t.layer3D}</span>
                </button>
              ))}
            </div>
          )}
          <button onClick={() => setShowLayerPicker(!showLayerPicker)} className="bg-white p-3 rounded-2xl shadow-2xl text-[#93132B] border border-gray-100 flex items-center gap-2">
      ...
      {isFinalized && (
        <div className="fixed inset-0 z-[5000] flex items-center justify-center p-6 bg-[#93132B]/20 backdrop-blur-md">
          <div className="bg-white rounded-[2.5rem] p-8 md:p-10 max-w-4xl w-full shadow-3xl flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                <div className="bg-[#93132B10] text-[#93132B] p-3 rounded-2xl"><svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg></div>
            ...
            {!isStored ? (
              <>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar space-y-8 pr-4 mb-8">
                  <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 relative">
                    ...
                    <div className="prose prose-sm text-gray-600 max-w-none">
                       {summaryEN.split('\n').map((line, i) => <p key={i} className="mb-2" dangerouslySetInnerHTML={{ __html: line.replace(/\*\*(.*?)\*\*/g, '<strong class="text-[#93132B]">$1</strong>') }} />)}
                    </div>
                  </div>
                ...
                <div className="flex gap-4">
                  ...
                  <button onClick={handleStoreData} disabled={isStoring} className="flex-[2] bg-[#93132B] text-white py-4 rounded-2xl font-bold hover:bg-[#7a0f24] shadow-xl flex items-center justify-center gap-3">
                    {isStoring ? <div className="animate-spin h-5 w-5 border-2 border-white border-t-transparent rounded-full" /> : t.storeData}
                  </button>
                </div>
              </>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                <div className="w-24 h-24 bg-green-100 text-green-600 rounded-full flex items-center justify-center text-5xl mb-6"><svg className="w-12 h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg></div>
                <h3 className="text-2xl font-black text-gray-900 mb-4">{t.successMessage}</h3>
                <p className="text-gray-500 mb-8 max-w-sm">{t.successDesc}</p>
